...
# This program calculates the state probability for the E-MEPI model based on DURACRETE2000
#------------------------------------------------------
rm(list=ls()) #: clear the memory and objects in R console
#------------------------------------------------------
#.....read inputparameters from file

input <- read.csv("02-EMEPI/02.0-input.csv", header =TRUE, sep = ";") #: import data from excel
...
haz <- array(dim=c((IM+JM-1),Omax)) #: calculating the hazard rate
for (o in 1:Omax){
  for (i in 1:(IM+JM-1)){
    haz[i,o] = 0
  }
...
for (o in 1:Omax){
  for (i in 1:(I[o]+J[o]-1)){
    haz[i,o] = length(which(det[,o]==i))
  }
}
for (o in 1:Omax){
  for (i in 1:(I[o]+J[o]-1)){
...
file.remove("02-EMEPI/02.1-pi1.csv")
file.remove("02-EMEPI/02.2-pi2.csv")
file.remove("02-EMEPI/02.3-pi3.csv")
file.remove("02-EMEPI/02.0-haz.csv")

file.create("02-EMEPI/02.1-pi1.csv")
file.create("02-EMEPI/02.2-pi2.csv")
file.create("02-EMEPI/02.3-pi3.csv")
file.create("02-EMEPI/02.0-haz.csv")

write.table(pi[,,1], file = "02-EMEPI/02.1-pi1.csv", sep = ";", append = TRUE,col.names = FALSE) 
write.table(pi[,,2], file = "02-EMEPI/02.2-pi2.csv", sep = ";", append = TRUE,col.names = FALSE) 
write.table(pi[,,3], file = "02-EMEPI/02.3-pi3.csv", sep = ";", append = TRUE,col.names = FALSE)
write.table(haz, file = "02-EMEPI/02.0-haz.csv", sep = ";", append = TRUE,col.names = FALSE)
#------------------------------THE END------------------------------