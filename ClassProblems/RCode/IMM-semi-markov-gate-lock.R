...
I2=1600 #impacts if both two fans are replaced and repaired.
...
p0=c(0.05,0.1,0.3,0.35,0.2)
q0=c(0.1,0.2,0.42,0.18,0.1)
...
O[3,2]=p[3,1]*q[4,2] +p[3,1]*sum(q[c(5:year),2]) +q[4,2]*sum(p[c(4:year),1])

#print(O[3,2])
#stop("debugg")

O[3,3]=p[4,2]*q[3,1]+p[4,2]*sum(q[c(4:year),1])+q[3,1]*sum(p[c(5:year),2])
O[3,4]=p[5,3]*q[3,1]+p[5,3]*sum(q[c(4:year),1])
O[3,5]=p[3,1]*q[5,3]+q[5,3]*sum(p[c(4:year),1])
O[4,1]=p[4,1]*q[4,1]+p[4,1]*sum(q[c(5:year),1])+q[4,1]*sum(p[c(5:year),1])
O[4,2]=p[4,1]*q[5,2]+q[5,2]*sum(p[c(5:year),1])
O[4,3]=p[5,2]*q[4,1]+p[5,2]*sum(q[c(5:year),1])
O[4,4]=0
O[4,5]=0
O[5,1]=p[5,1]*q[5,1]
O[5,2]=0
