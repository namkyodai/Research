...
param Maxcoverage; 
param Maxprofit; 
param w1; #weight for coverage
param w2; #weight for profit
...
# Objective function 
minimize weight: (w1/Maxcoverage)*(Maxcoverage-sum{i in 1..1,j in 1..1}x[i,j])+ (w2/Maxprofit)*(Maxprofit-sum{i in 1..T,j in 1..T}(x[i,j]*R[i,j]-y[i,j]*c)); 
