...
mtext(expression(paste("Probability")),side=2,col="black",line=2.5)  
box()
...
par(new=TRUE)
plot(cdfweib(x,alpha,m),pch=22,axes=FALSE,ylim=c(0,1),ylab="", xlab="",col="blue",type="b",lwd=1,cex=0.5)
grid(10, 10, col = "lightgray", lty = "dotted",lwd = par("lwd"), equilogs = TRUE)




colors=c("red","violet","blue")

legend("topright",inset=.05,legend=c("Reliability", "Failure density","Failure probability"),text.col="black",lty=c(1,3,1),pch=c(14,12,22),col=colors, horiz=F,cex=0.8)
