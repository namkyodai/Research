...
cci<-2
maxm<- 2.6
step=seq(1,maxm, by =0.01)
msa<- matrix(double(1),nrow=length(step),ncol=1)
...
plot(msa, Min_Csa, pch=2, axes=FALSE, ylim=c(0,50), xlab="", ylab="", 
...
axis(1,pretty(range(msa),20))
box()

par(new=TRUE)

## Plot the second plot and put axis scale on right
plot(msa, T_optimalsa, pch=4,  xlab="", ylab="", ylim=c(0,50), 
...
mtext(expression(paste("Deterioration parameter ", m)),side=1,col="black",line=2.5)  
