...
step=seq(1,MaxCS, by =1)


CSbandau<-matrix(double(1),nrow=MaxCS+1,ncol=6)
CSbandau[1,]<-c(1,0,0,0,0,0)
CSbandau[2,]<-c(0,1,0,0,0,0)
CSbandau[3,]<-c(0,0,1,0,0,0)
CSbandau[4,]<-c(0,0,0,1,0,0)
CSbandau[5,]<-c(0,0,0,0,1,0)
CSbandau[6,]<-c(0,0,0,0,0,1)


#rhosa<- matrix(double(1),nrow=length(step),ncol=1)
T_optimalsa<-matrix(double(1),nrow=MaxCS,ncol=1)
Min_Csa<-matrix(double(1),nrow=MaxCS,ncol=1)

for (u in 1:MaxCS){
  initialCS<-CSbandau[u,]
...
plot(step, Min_Csa, pch=2, axes=FALSE, ylim=c(6.5,7), xlab="", ylab="", 
...
plot(step, T_optimalsa, pch=4,  xlab="", ylab="", ylim=c(16,19), 
     axes=FALSE, type="l", col="coral1",lwd=5,lty=2)
## a little farther out (line=4) to make room for labels
mtext("ORT (years)",side=4,col="black",line=3.5) 
axis(4, ylim=c(0,20), col="black",col.axis="black",las=1)
mtext(expression(paste("Condition state of the bridge ", )),side=1,col="black",line=2.5)  
