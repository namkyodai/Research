...
maxCIimpact<-1000 #corrective intervention cost for the bridge
cci<-2
step=seq(0.1,maxCIimpact, by =1)
ccisa<- matrix(double(1),nrow=length(step),ncol=1)
CIimpactsa<- matrix(double(1),nrow=length(step),ncol=1)
